...
- `api/event.js` סופר שימוש בדליים גסים (סוג שיטה, מספר מערכות, טווח מחיר). **מסרב** לשמות לקוחות, לטקסט חופשי, למחירים מדויקים ולתוכן ההצעה. המסמך לעולם לא עוזב את הדפדפן.
  **והוא לא צובר כלום.** הוא כותב שורה אחת ל-stdout, ושמירת הלוגים אצל המאחסן היא שעה עד יום לפי התוכנית — כלומר אירוע נמחק מהר יותר משהוא נצבר, ולשאלה היחידה שהקובץ קיים בשבילה, האם מישהו משתמש, אין חודש לקרוא. זה לא באג; זה המצב, והוא מוצהר מבחוץ ולא רק בקוד. חיבור מאגר אמיתי הוא החלטה של מי שמפעיל, לא פער שנסגר בשקט.
...
- `api/health.js` מדווח אם `POSTCALL_KEYS` מוגדר. בלי זה, שכחה להגדיר את המשתנה מכבה את השער בשקט על פריסה חיה, ואין דרך לראות את זה.
  הוא מדווח גם `telemetryDurable`, שהוא `false` כל עוד היעד של `api/event.js` הוא שורת לוג בלבד — אותו היגיון בדיוק, על השאלה השנייה. הדגל **קשור לקוד שהוא מתאר**: `api/health.test.js` קורא את `event.js` ונכשל ברגע שהשניים לא מסכימים, בשני הכיוונים. `false` שנכתב ביד הוא בדיוק מה שנשאר `false` שנה אחרי שמישהו חיבר מאגר — או גרוע מזה, מתהפך ל-`true` בלי שהיעד השתנה.
...
4. **שלב 3 · הצד השני** — הדביקו פרופיל לינקדאין או השתמשו בפרומפט למחקר מעמיק.
5. **שלב 4 · התסריט** — שאלות עומק, סדר מחייב, נקודות עצירה, וכרטיס קריאה שמתרגם תשובות נפוצות לסוג ההצעה המתאימה.
