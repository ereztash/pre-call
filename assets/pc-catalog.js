  ...
  const TEMPLATES = [
    {
      ...
      note: 'שלושה סעיפים זזו. מקרי קצה עברו ל"בתוספת" ולא ל"לא כלול", וזו ההחלטה החשובה כאן: הזמנה שמגיעה כטקסט חופשי בוואטסאפ נכתבת בכל פעם אחרת, ולכן החריגים אינם תקלה שצריך להחריג אלא עבודה שאפשר למכור. ההזמנות ההיסטוריות בגיליון כמעט תמיד צריכות המרה לפני ההרצה הראשונה, גם זו עבודה בפני עצמה. והתאמות אחרי שינוי ב-API — ספקי וואטסאפ משנים תדיר, ועדיף למכור את זה כתחזוקה מאשר להחריג ולריב עליו בעוד חצי שנה.'
    },
    {
      id: 'leads',
      ...
      note: 'ניטור עבר ל"כלול", בניגוד לברירת המחדל. ליד שנופל בשקט לא מייצר תלונה — הוא פשוט לא מגיע, ואחרי חודש הלקוח מסיק שהפרויקט לא עבד. בתהליך לידים הניטור הוא מה שמוכיח שהוא כן. ייבוא רשימת הלידים הקיימת מוצע בתשלום, כי זו עבודה בפני עצמה. שים לב גם לסעיף הפרימיום: חיבור ל-Salesforce או HubSpot דורש מסלול יקר יותר בכלי האוטומציה, וזו עלות חוזרת של הלקוח.'
    },
    {
      id: 'collect',
      ...
      note: 'ניטור נכלל כאן ולא נמכר בנפרד. הרצת גבייה שנעצרת לא מייצרת שגיאה שמישהו רואה — היא מייצרת כסף שלא נכנס, וזה מתגלה חודש אחרי. התאמת החשבוניות הפתוחות לפני ההרצה הראשונה מוצעת בתשלום. ושים לב לסעיף "שינוי בתהליך העסקי" שנשאר מוחרג: "מתי שולחים תזכורת שנייה" היא החלטת מדיניות של הלקוח, לא באג שלך.'
    },
    {
      id: 'report',
      ...
      note: 'ניקוי הנתונים ההיסטוריים עבר ל"בתוספת" ולא ל"לא כלול". בדוחות זה כמעט תמיד נדרש, וזו עבודה בפני עצמה — עדיף להציע אותה מאשר לבלוע אותה או לגלות אותה באמצע. גם התאמות אחרי שינוי במקורות הנתונים נמכרות ולא רק מוחרגות: דוח שואב מכמה מערכות, וכל אחת מהן משתנה מתישהו.'
    },
    {
      id: 'booking',
      ...
      note: 'ההדרכה עברה ל"בתוספת". בקבלה ובתיאום יש תחלופת אנשים, ו"הדרכה אחת עד שעה" הופכת לשלוש שיחות בלי שאף אחד החליט על זה — עדיף למכור ליווי מאשר לספוג אותו. גם ייבוא היומן ואנשי הקשר הקיימים מוצע בתשלום. ולגבי המספרים: עלות אי-הגעה היא מה שמצדיק את הפרויקט וגם מה שהלקוח הכי נוטה לנחש. שאל כמה פגישות התפספסו בחודש שעבר, לא כמה "בדרך כלל".'
    }
  ];
