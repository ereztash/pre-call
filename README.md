...
להפעלה: הריצו `node tools/mint-key.js 5 --env`, הגדירו את `POSTCALL_KEYS` ב-Vercel בשורה שיצאה, ושלחו לכל קונה מפתח אחד. `PAYMENT_URL` ב-`assets/pc-gate.js` הוא עדיין מציין מקום — הכפתור אומר את זה במפורש במקום להעמיד פנים.
