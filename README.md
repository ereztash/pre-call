...
שני כלים לשיחת אפיון עם לקוח. **POST-CALL** (`post-call.html`) הוא הכלי הראשי: הוא לוקח את מה שנאמר בשיחה והופך אותו להצעת מחיר מתומחרת. **PRE-CALL** (`index.html`) הוא הכלי המוקדם יותר, שמכין את השאלות לפני השיחה.
...
assets/*.test.js      395 בדיקות שרצות ב-Node בלי דפדפן
...
**395 בדיקות בשמונה-עשרה חבילות, רצות ב-Node בלי דפדפן ובלי תלויות.** הן לא בודקות רק חישוב:
