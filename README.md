...
> קוד קנייני, לא קוד פתוח. הריפו גלוי כדי שאפשר יהיה לקרוא אותו, וזה לא רישיון — [התנאים המלאים](#רישיון). רישוי: erez2812345@gmail.com

**מה זה מנסה להזיז: את שיעור הצעות המחיר שנסגרות.**

שני רבדים מתוך זה בנויים כאן: **המחיר**, שנגזר ממספר שהלקוח עצמו אמר ומצוטט עם המשפט שממנו נלקח, ו**המסמך**, שהגבולות והתנאים בו נגזרים מאותן תשובות. מה שלא נבנה, ובכוונה, הוא עיצוב חזותי — גופן, אורך, מבנה. `docs/words.md` כבר תמחר מנופים בפרויקט הזה פעם אחת ומצא שהמנוף שביקשתי למשוך הוא החלש בלוח; סדר הרבדים כאן נגזר באותה שיטה, והוא הערכה ולא מדידה.

**ומה שלא נטען: שהכלי מעלה את שיעור הסגירה.** שיעור הוא פרופורציה, והרמה של עשר נקודות דורשת כ-660 הצעות מוכרעות כדי בכלל להיראות — אחת-עשרה שנה אצל מי ששולח חמש בחודש. מה שמובטח הוא מה שנבדק כבר בשימוש הראשון: מהשיחה למסמך מתומחר שאפשר להגן עליו בחדר. `assets/pc-history.js` מסרב לטעון יותר מזה גם על המסך, ולא רק כאן.

**שלושה קבצים שמראים איך מתקבלות החלטות כאן:**

| | |
|---|---|
| [`docs/words.md`](docs/words.md) | תמחור של מילה ביחידות של אחוזי השלמה. המסקנה סתרה את הבקשה שממנה התחילה: קיצור טקסט הוא המנוף החלש בלוח, ושדה אחד שמוסר שווה עד פי 42 ממאה מילים |
| [`docs/first-ten.md`](docs/first-ten.md) | למה חמישה משתמשים אינם מחקר. כיסוי במקרה הגרוע: 55% בחמישה, 80% בעשרה |
| [`docs/design-persona.md`](docs/design-persona.md) | "צפוף וצורם" הפך לארבעה מספרים עם ספים. ארבע חריגות מדודות; היום הבדיקה עוברת, ומפילה את הבילד אם אחת מהן תחזור |

---

## הרבדים, ומה בנוי מהם

זה הסדר שהפסקה למעלה מפנה אליו. הוא מסודר לפי אפקט משוער על שיעור הסגירה, לא לפי קלות בנייה, והוא הערכה — אין כאן מדידה של אף שורה:

| רובד | מה הוא עושה | מצב |
|---|---|---|
| **המחיר** | נגזר ממספר שהלקוח אמר, עם המשפט שממנו נלקח | בנוי |
| **המסמך** | גבולות, תוקף ותנאים נגזרים מאותן תשובות | בנוי |
| **המהירות** | מהשיחה להצעה שנשלחה | נמדד בפנקס, לא מנוהל |
| **המעקב** | הצעה ששקטה, ומתי לחזור אליה | `pc-followup.js` · חלקי |
| **הבהירות** | מה לא כלול, ובכמה אופציות | קטלוג הסקופ · חלקי |
| **ההצגה** | גופן, אורך, מבנה חזותי | לא נבנה |

שתי השורות התחתונות הן היחידות שאפשר לבנות בלי לדעת דבר על הלקוח, וזו בדיוק הסיבה שהן למטה.

**הרבדים שלא נבנו הם למה הפרויקט ממשיך.** הטיעון המלא — למה שיעור סגירה ולא חיסכון זמן, מה החשבון אומר על מה שאי אפשר להבטיח, מאיפה יגיע כסף, ומה יפריך את כל זה — ב-[`docs/strategy.md`](docs/strategy.md). שם מותר לו להיות לא מדוד; כאן לא.

## מהשיחה להצעה

זה ה-MVP הראשון של מערכת שנועדה לשפר את **תנאי הסגירה** של שירותים מותאמים: הכנה לשיחת אפיון, והצעה שמחזירה ללקוח את הכאב והערך במילים שהוא עצמו אמר.

המערכת לא מבטיחה שהצעה תיסגר. תקציב, תחרות, איכות הביצוע ומי יושב בחדר נמצאים מחוץ לשליטתה. היא מתמקדת במה שהמוכר כן יכול לשפר ולבדוק: איכות הגילוי, מקור המספרים, בהירות ההצעה, ההיקף, המחיר, וההמשך אחרי השליחה.

**המוצר מיועד למוכרי שירותי אוטומציה וייעול תהליכים.** זה לא קהל שנבחר לצורך מסר: מנוע הערך נשען על תהליך שניתן לכמת — זמן שנחסך, עלות שעה ותקלות — ועל תבניות הסקופ של הוורטיקל הזה. למי שמוכר שירות שלא מתכמת כך, המחיר כאן ייצא מהשיטות האחרות והטענה המרכזית של הכלי לא תעבוד.

| חלק | השאלה שהוא פותר | מצב |
|---|---|---|
| **מוצר 1 · PRE-CALL / POST-CALL** | איך מגיעים משיחת אפיון להצעה שאפשר להסביר ולהגן עליה? | **קיים.** הכנה, הצעה, שליחה ופנקס מקומי |
| **מוצר 2 · מרחב החלטה ללקוח** | איך הופכים הצעה שנשלחה לבחירה בין חלופות, במקום מיקוח על מחיר? | **עתידי.** לא קיים בגרסה הזו, ולא מפורט כאן מעבר לעקרון |
| **למידה** | מה בהצעה ובמחיר החזיק, ומה ויתר? | **חלקי, ובשני כיוונים שונים.** ההיסטוריה המסחרית כן חוזרת: הפנקס משנה את שורות הפתיחה של התסריט בשיחה הבאה (`pre-call.js:513`). כיול האומדן — שעות מוערכות מול בפועל — נשאר בתוך POST-CALL |

| | POST-CALL | PRE-CALL |
|---|---|---|
| מתי | אחרי שיחת האפיון | לפני שיחת האפיון |
...
POST-CALL נבנה כתשובה לכשל מבני ב-PRE-CALL: עוגן המחיר שלו נשען על העסקה האחרונה של המשתמש, ולכן הוא ריק בדיוק אצל מי שהכלי מיועד לו — מי שעוד לא סגר עסקה. לכן POST-CALL הוא הכלי הראשי, ו-PRE-CALL מכין את השיחה שממנה יוצאים המספרים.
...
**סטטוס.** הזרימה נבדקה מקצה לקצה, כולל המסע המלא בשלושה מנועי דפדפן. **טרם השלים אותה משתמש זר:** שלושת המבקרים הראשונים נחסמו בקיר login של פריסת preview, ו-[`docs/first-ten.md`](docs/first-ten.md) מחזיק את הפרוטוקול לעשרה הראשונים — ושלושת החסומים הם שלושה מהם. זו נקודת ההתחלה של הפיילוט, לא תוצאה שלו.

**מחפש שותפי פיילוט.** אם אתם מוכרים אוטומציה וייעול תהליכים ויש לכם שיחת אפיון בשבועות הקרובים — [דברו איתי בוואטסאפ](https://wa.me/972524545963), או `+972 52-4545963`. המוצר פתוח לניסיון ב-[pre-call-swart.vercel.app](https://pre-call-swart.vercel.app/).
...
| **מי אמר — נגזר כשאפשר, נשאל כשלא** | הגזירה היא מסימוני דובר בתמלול, וכלי תמלול לא כותבים אותם: מתוך שנים-עשר תמלולים אמיתיים, אפס נושאים סימון. בלי סימון כל מספר חוזר `unknown`, ה-provenance מתמוטט ל-`mine`, והכלי מכבה בשקט את הטענה הכי חזקה שיש לו. לכן השאלה מופיעה **על השורה, ליד המשפט** — המקום היחיד שאפשר לענות בו בלי לזכור כלום. נגזר כשאפשר, נשאל כשלא, אף פעם לא מונח |
...
**נפילה לאחור מקומית — שלושה שלבים, לא אחד.** בלי מודל בכלל:

1. **מספרים מדוברים הופכים לספרות.** דיבור אמיתי אין בו ספרות. "כחמישים הזמנות ביום" הוא מה שכלי תמלול מחזיר, ואף רמז שבנוי סביב `\d+` לא היה רואה אותו. ההמרה משמשת **להתאמה בלבד** — הציטוט נשאר המשפט כפי שנאמר, מילה במילה (`assets/pc-numerals.js`).
2. **סולם הראיות מחליט אילו רמזים בכלל מותר לחפש.** לא "מה נמצא" אלא "מה מותר לקרוא": חמישה שלבים, מהחזק לחלש, וכל שלב מצהיר אילו רמזים הוא מרשה. בשיחה שאין בה תהליך חוזר, הרמז של "כמה עולה תקלה" לא סתם לא בשימוש — הוא לא מחופש. השורה התחתונה תמיד מחזיקה (`assets/pc-ladder.js`).
3. **רמז חלש לא קורא משפט ששייך לרמז אחר.** "תשעים שקל לשעה" הוא תעריף, לא עלות של טעות — וכשהשלב הפעיל מכבה את רמז המחיר, הרמז החלש בקובץ היה אוסף אותו בלי מתחרה. הסירוב יושב על הרמז עצמו, בכל שלב.

חלש יותר ממודל, ומסומן ככזה בכל שורה.
...
שני מתגים בראש כל דף, ושניהם עובדים באותו עיקרון: ברירת המחדל לא מחליטה בשבילכם.

**מצב כהה ובהיר.** ברירת המחדל עוקבת אחרי מערכת ההפעלה; לחיצה מקבעת כהה, עוד אחת בהיר, ושלישית חוזרת לאוטומטי. הערכה הכהה איננה פלטה שנייה — היא אותם 44 צבעי הסולם, נקראים מהקצה השני (`assets/theme.css`), כך שהמרחק בין כל שני צעדים שורד את ההיפוך. `tokens.test.js` נועל את המראה, וגם ביקורת העיצוב וגם axe רצים פעמיים — פעם לכל ערכה. הדפסה תמיד בהירה: נייר הוא נייר.

**עברית ואנגלית.** העברית בקבצים היא המקור; האנגלית היא שכבת מילון שמפתחו הוא המשפט העברי עצמו (`assets/pc-i18n.js`). מבקר בעברית לא מוריד את המילונים בכלל — הם נטענים רק אחרי בחירה באנגלית, ולכן תקציבי המשקל לדף נשארים כנים לגבי ביקורת ברירת המחדל. `assets/i18n.test.js` נכשל על כל מחרוזת עברית שנשלחת בלי תרגום, כך שעריכת טקסט לא יכולה לשכוח את השפה השנייה בשקט. המסמכים שהכלי מייצר — התסריט וההצעה — עוקבים אחרי שפת הממשק: מפעיל באנגלית שולח הצעה באנגלית.
...
| קובץ יומן שהורדתם | במחשב שלכם, וביומן שלכם | ידנית · הכלי לא רואה אותו |
| פרופיל העסק (PRE-CALL) | `localStorage` · `precall_profile_v1` | בכפתור מחיקה |
| פרטי הצד השני (PRE-CALL) | בזיכרון הטאב בלבד | בסגירת הדף |

**שני יוצאי דופן, ושניהם בבחירה שלכם.** הראשון: קובץ יומן. אם תלחצו "הורד קובץ יומן" בשלב 3 של PRE-CALL, שם העסק או האדם נכתב בכותרת האירוע — כדי שהתזכורת תדע על מה היא — והקובץ יורד למחשב שלכם ונכנס ליומן שלכם. הוא לא עובר דרכנו, ובלי הלחיצה השם לא יוצא מהטאב. שם הקובץ עצמו הוא ASCII בלבד (`call-2026-08-20.ics`), כי דפדפן מפיל שם קובץ עברי לגמרי ושומר אותו בלי סיומת — נמדד, לא הונח.

והשני: **התמלול, וההחלטה שלכם.** אם תבחרו להריץ את פרומפט החילוץ ב-Claude או ב-ChatGPT, התמלול עובר לשם — לא דרכנו. הכלי כותב את הפרומפט; לאן הוא הולך זו בחירה שלכם. הנפילה-לאחור המקומית עובדת בלי שום רשת.
...
assets/pc-ladder.js   סולם הראיות — על מה השיחה מרשה להישען, ואילו רמזים
                      מותר לקרוא בכלל, טהור
assets/pc-numerals.js מספרים מדוברים לספרות — להתאמה בלבד, אף פעם לציטוט, טהור
...
assets/pc-ical.js     פרימיטיבים של iCalendar + קובץ היומן לשיחה עצמה, טהור
assets/pc-followup.js מתי הצעה שנשלחה שותקת, והתאריך שהלקוח נקב, טהור
assets/pc-proposal.js המסמך עצמו, פונקציה טהורה של הקשר
assets/pc-commitments.js  מה השיחה כבר קבעה, ומה השאלה הבאה ששווה לשאול, טהור
assets/pc-backup.js   ייצוא ושחזור של כל מה שהדפדפן מחזיק, טהור
assets/pc-contact.js  מסלול הקנייה היחיד — לכל מקום ששולח קונה החוצה
assets/pc-boot.js     ערכה וכיוון לפני הצביעה הראשונה
...
assets/pre-call-content.js  שני הפרומפטים של PRE-CALL — טקסט, מופרד מהרינדור
assets/pre-call.js    הקליפה של PRE-CALL — כולל מצב שיחה
assets/post-call.js   הקליפה — קריאת DOM, מצב, חיווט

landing/post-call.html  חוזה היעד — מה המוצר אמור לדעת לעשות. לא נפרס
                      עד שכל claim נדרש מקבל בדיקת מוצר עוברת

assets/*.test.js      בדיקות שרצות ב-Node בלי דפדפן
assets/landing.test.js   כל claim בחוזה מול הבדיקה שמוכיחה אותו, ושער השחרור
assets/journey.test.js   המסע המלא, בדפדפן אמיתי, בשלושה מנועים
assets/post-call.test.js  החיווט של POST-CALL — מה שרק לחיצה מגיעה אליו
assets/pre-call-page.test.js  החיווט של PRE-CALL, כולל מצב שיחה
tools/page-harness.js    שרת סטטי + פרופיל דפדפן לבדיקה, משותף לשתיהן
...
**בדיקות ב-35 חבילות, רצות ב-Node בלי דפדפן ובלי תלויות** — ומעליהן חמש חבילות שכן דורשות דפדפן: המסע המלא בשלושה מנועים (Chromium, Firefox, WebKit), החיווט של שני הכלים — הפונקציות שרק לחיצה מגיעה אליהן, כולל מצב שיחה, ו-axe-core מול WCAG 2.1 AA על כל דף בכל מצב, ומדידת Core Web Vitals על טלפון עם ויסות מעבד פי 4 — שלושה מדגמים לכל דף, החציון מכריע, והטווח מודפס. מדגם בודד הכריע כאן קודם. על קוד זהה נמדד ביום אחד טווח של 141 עד 288ms מול תקציב 200 — כלומר מצב המכונה מכריע, ולא הדף. גם כשל וגם מעבר שווים כאן מעט, ולכן הטווח מודפס ולא רק הפסיקה.
...
| צבע | 94 ליטרלים, 9 משפחות גוון | 46 ליטרלים, 4 עוגנים | כל צבע על עוגן; שני צעדים באותה בהירות נושאים אותה רוויה; **ולא כותבים הקסה שיש לה שם** |
...
**הסולם הוכרז ואז נעקף 239 פעם.** כל מה שלמעלה בדק את הסולם עצמו, ושום דבר לא
בדק שמשתמשים בו — וזה בדיוק מה שקרה: 44 צעדים מסודרים, וכל כלל בגיליון נוקב את
הצבע שלו בהקסה כאילו הסולם הוא תיעוד. שני הדברים היו נכונים בבת אחת, כולל
המשמעות המעשית: שינוי `--nt-9` לא שינה כלום. זה גרוע יותר מהיעדר סולם, כי זה
נקרא כמו סולם — ורִיבְּרֶנְד היה נשאר 239 עריכות ידניות.

התיקון מכני, והמיגון עליו הוא מדידה ולא ביטחון: 239 ההחלפות צולמו לפני ואחרי,
ו-236 מהן יצאו **זהות לפיקסל**. שלוש זזו בכוונה — שלושת הצבעים שהוספתי ב-`eeafb52`
ולא היה להם שם על הסולם. אחד מהם היה גם פגם: סמן ה-`—` ברשימת מה-שלא-נכלל נמדד
**4.11:1** מול נייר המסמך, מתחת ל-AA, כשהטקסט שהוא מסמן יושב על 5.21:1 — כלומר
הדבר החלש ביותר על הפריט האחד שיוצא מהבית. axe לא רואה אותו, כי axe לא מעריך
תוכן של `::before`. הגיליון גדל 2.4KB גולמי ו-382 בייט דחוסים בלבד, כי
`var(--nt-9)` חוזר על עצמו נדחס טוב יותר מאותה הקסה חוזרת.
...
בונה תסריט שאלות לשיחת אפיון עבור מוכרי שירותי אוטומציה וייעול תהליכים, לפני שכותבים הצעת מחיר. כלי HTML עצמאי, בלי בילד ובלי שרת.
...
2. **שלב 1 · הפרופיל העסקי** — ארבעה שדות, פעם אחת, נשמר בדפדפן. חמישה שדות מחדדים ופרומפט למילוי מהיר ב-AI ממתינים מאחורי שני חשיפונים — אופציה, לא הוראה. (עד כאן זה היה שני שלבים שנפתחו בהוראה לצאת ל-Claude או ל-ChatGPT; מודל הקשב תמחר את היציאה הזאת ב-35% מהמגיעים, והמבנה הוחלף. docs/words.md מחזיק את הלפני/אחרי.)
3. **שלב 2 · הצד השני** — הדביקו פרופיל לינקדאין או השתמשו בפרומפט למחקר מעמיק, ונסחו את המשפט שתגידו בשאלה 10.
4. **שלב 3 · התסריט** — שאלות עומק, סדר מחייב, נקודות עצירה, וכרטיס קריאה שמתרגם תשובות נפוצות לסוג ההצעה המתאימה. יש לו שתי תצוגות: הכנה, ומצב שיחה.
...
- **טרם תועד מסלול שהושלם בידי משתמש זר.** זו טענת סטטוס פיילוט, לא טענה על איכות המוצר ולא מדד סגירה. הפרוטוקול לעשרת הראשונים ב-[`docs/first-ten.md`](docs/first-ten.md).
- **לולאת הלמידה סוגרת מעגל אחד ולא שניים.** מה שכן חוזר הוא ההיסטוריה המסחרית: `pre-call.html` טוען את `deals.js`, ו-`pre-call.js:513` קורא ל-`PC.deals.tenure()` כדי לשנות את מה שהתסריט אומר לכם לפני השיחה הבאה — ירדתם במחיר לפני שנשאלתם, אף מחיר לא זז ולכן לא נבדקה תקרה, או שיש הצעות ואין סגירה ולכן אין לכם עוגן משלכם. מה ש**לא** חוזר הוא כיול האומדן: השעות בפועל מתקנות את טבלת האומדן בתוך POST-CALL ואינן מגיעות ל-PRE-CALL.

  *גרסה קודמת של השורה הזאת אמרה שתוצאת עסקה אינה חוזרת ל-PRE-CALL כלל. זה היה שגוי — הבדיקה שמאחוריה חיפשה מפתחות `localStorage` ולא קריאות למודול, והלולאה עוברת דרך ה-API של `deals.js`. `assets/pre-call.test.js` מכסה אותה מאז שהיא נכתבה, עם `deals.js` אמיתי ולא מזויף.*
...
**מותר** לקרוא, להריץ מקומית להתרשמות, ולצטט קטעים עם ייחוס. **אסור** להשתמש כדי
לתת שירות, להעתיק, לשנות, לפרוס, או לקחת את מודל התמחור, מודל האומדן, קטלוג
הסקופ או הנימוקים שבהערות אל תוך מוצר אחר. הנוסח המחייב ב-[LICENSE](LICENSE).
